...
public class SearchWorkOrder {
	boolean test = false;
	String workorder = GenerateData.generateRandomString(5);
	...
	//Fill an work order title
	   try{
		...
		//Choose device
	     ...
	     //Search the work order created earlier
	        try{
			Thread.sleep(1500);
			WebElement orgdrop = driver.findElement(By.id("selectView"));
			Select org = new Select(orgdrop);
			org.selectByVisibleText("All work order");
	        }catch(Exception e)
	        {
	        System.out.println(e);
	        }
	        ...
	        try{
	    	Thread.sleep(1500);
	    	driver.findElement(By.name("search")).sendKeys(workorder);
	    	}catch(Exception e)
	    	{
	    	System.out.println(e);
	    	}
